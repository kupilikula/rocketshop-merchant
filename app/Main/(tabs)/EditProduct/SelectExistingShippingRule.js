...
export default function SelectExistingShippingRule() {
    ...
    const handleConfirm = () => {
        ...
        dispatch(updateEditProductField({ field: 'shippingRuleDraft', value: shippingRules.find((r) => r.shippingRuleId===localSelectedRuleId)}));
                            ...
                            right={(props) => (
                                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                    {rule.similarityScore >= 20 && (
                                        <Badge style={{ backgroundColor: theme.colors.secondary, marginRight: 8, borderRadius: 0 }}>
                                            Best Match
                                        </Badge>
                                    )}
