...
const ListElement: React.FC<ListElementProps> = React.memo((collection) => {
    ...
    return (
        <ReorderableListItem>
            <Pressable onLongPress={drag} onPress={() => {router.push('/Main/(tabs)/Collections/Collection/'+collection.collectionId)}}>
...
const CollectionsScreen = () => {
    ...
    return (
        <Surface style={{paddingHorizontal: 10, backgroundColor: theme.colors.surface}}>
            <ReorderableList
                style={{}}
                data={data}
                ...
                <>
                    <View style={{marginVertical: 10}}>
                        <View style={{marginLeft: 8, marginTop: 8, display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-start'}}>
                            <MaterialIcons name={'category'} size={44} color={theme.colors.primary} style={{}}/>
                            <Text variant={'displaySmall'} style={{marginLeft: 10, color: theme.colors.secondary}}>Collections</Text>
                        </View>
                        <Text variant={'bodyLarge'} style={{marginLeft: 10}}>{nActive.toString() + ' Active Collection' + (nActive !== 1 ? 's' : '')}</Text>
                        <Text variant={'bodyLarge'} style={{marginLeft: 10}}>{nInactive.toString() + ' Inactive Collection'  + (nInactive !== 1 ? 's' : '')}</Text>
                    </View>
