...
import {TextInput, Button, RadioButton, Checkbox, Chip, useTheme, Text, Surface, Switch} from 'react-native-paper';
...
const PublishOfferScreen = () => {
    ...
    const [applicableTo, setApplicableTo] = useState(offer.applicableTo);
    const [conditions, setConditions] = useState(offer.conditions);
    const [validity, setValidity] = useState(offer.validity);
    const [showStartDatePicker, setShowStartDatePicker] = useState(false);
    const [showEndDatePicker, setShowEndDatePicker] = useState(false);
    const [productPickerVisible, setProductPickerVisible] = useState(false);
    const [collectionPickerVisible, setCollectionPickerVisible] = useState(false);
    const [tagPickerVisible, setTagPickerVisible] = useState(false);
    const [selectedProducts, setSelectedProducts] = useState(offer.applicableTo.products);
    const [selectedCollections, setSelectedCollections] = useState(offer.applicableTo.collections);
    const [selectedTags, setSelectedTags] = useState(offer.applicableTo.tags);
    const existingTags = ['Best Sellers', 'Featured', 'Discount', 'New Arrival', "Hand Made", "Organza"];
    const [offerStatus, setOfferStatus] = useState(offer.isActive);
    ...
    return (
        ...
        <ScrollView style={styles.container} contentContainerStyle={{
            ...
            <View style={styles.statusContainer}>
                <Text style={[styles.sectionTitle, {marginRight: 15}]}>Offer Status</Text>
                ...
                <Chip textStyle={{color: 'white', textAlign: 'center'}}
                      style={{marginLeft: 10, backgroundColor: offerStatus ? theme.colors.success : "#aaaaaa"}}>
