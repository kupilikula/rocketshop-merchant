...
export default function StoreFrontCollectionCard(props) {
    ...
    return <Card style={{
        ...
        <View style={{display: 'flex', flexDirection: 'row', justifyContent: 'space-between', width: '100%'}}>
            ...
            <View>
                <Link href={'/Main/StoreFront/Collections/Collection/' + props.collection.collectionId}><Text variant={'bodyLarge'}>See all</Text></Link>
            </View>
        </View>
        <View style={{alignSelf: 'center', flexDirection: 'row', flexWrap: 'wrap', display: 'flex', width: '100%', justifyContent: 'center'}}>
            {props.collection.products.slice(0,props.collection.storeFrontDisplayNumberOfItems).map( (p) => {
