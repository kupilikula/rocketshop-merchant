...
export const CustomerListItem = ({customer}) => {
    ...
    const mostRecentOrderDate = new Date(Math.max.apply(null, customer.orders.map((o) => o.orderDate)));

    return <Pressable onPress={() => router.push('/Main/(tabs)/Customers/Customer/' + customer.customerId)}>
